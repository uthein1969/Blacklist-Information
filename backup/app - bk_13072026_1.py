...
def get_google_sheet():
    try:
        # 💡 gspread ဗားရှင်းအသစ်များအတွက် အားထားရဆုံး အမှန်ကန်ဆုံး Auth စနစ်ပုံစံ ဖြစ်ပါတယ်ဗျာ
        scopes = [
            "https://www.googleapis.com/auth/spreadsheets",
            "https://www.googleapis.com/auth/drive"
        ]
        creds = ServiceAccountCredentials.from_json_keyfile_name(
            "gen-lang-client-0490646413-a25b1a1118b6.json", 
            scopes
        )
        client = gspread.authorize(creds)
        return client.open("Blacklist_Information")
    except Exception as e:
        # Error တက်ပါက Streamlit Cloud Terminal တွင် မြင်တွေ့နိုင်ရန်
...
def auto_sync_append_record(data_dict):
    ...
    if spreadsheet:
        try:
            ...
            # 💡 Payload ထဲမှ Key နာမည်များကို ကွက်တိ ဆွဲထုတ်ခြင်း
            row_value = [
...
def auto_sync_update_record(record_id, updated_data_dict):
    ...
    if spreadsheet:
        try:
            ...
            # Column A (blacklist_id) တစ်ခုလုံးကို ဆွဲထုတ်၍ ရှာဖွေခြင်း
            id_list = worksheet.col_values(1)
            str_id = str(record_id)
            
            if str_id in id_list:
                row_index = id_list.index(str_id) + 1 # Row index အား ကွက်တိရှာခြင်း
                
                # Column A မှ F အစီအစဉ်အတိုင်း တန်ဖိုးအသစ်များ နေရာချခြင်း
                row_value = [
                    str_id,
                ...
                # 🎯 gspread ဗားရှင်းသစ် အကြံပြုချက်အတိုင်း range_name နှင့် values နေရာ ကွက်တိ ပြင်ဆင်ခြင်း
                ...
                # ရှာမတွေ့ပါက ဒေတာမပျောက်ပျက်စေရန် အောက်ဆုံးတွင် Append လုပ်ပေးခြင်း
                    ...
                    # 👉 [ခ] Edit Mode ဖြစ်နေလျှင် (ဝန်ထမ်း အချက်အလက်ပြင်ဆင်ရန် Form ပြသခြင်း)
                        ...
                        if update_submitted:
                            ...
                            try:
                                ...
                                # 💡 Error တက်လာပါက Screen ပေါ်တွင် ချက်ချင်း လိမ္မော်ရောင်ဘားဖြင့် ပြသစေခြင်း
