...
# Google Sheets Authentication အတွက် ကြိုတင်ဆောက်ခြင်း
def get_google_sheet():
    try:
        scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
        creds = ServiceAccountCredentials.from_json_keyfile_name("gen-lang-client-0490646413-a25b1a1118b6.json", scope)
        client = gspread.authorize(creds)
        return client.open("Blacklist_Information")
    except Exception as e:
        print(f"Google Sheet Auth Error: {str(e)}")
        return None

# 🌟 ၁။ Data Add New ရောက်လာလျှင် Google Sheet ၌ အောက်ဆုံးတွင် တန်းထည့်မည့် စနစ်သစ်
def auto_sync_append_record(data_dict):
    ...
    if spreadsheet:
        try:
            ...
            # 💡 လူကြီးမင်း၏ Google Sheet Column (A, B, C, D, E, F) အစီအစဉ်အတိုင်း ကွက်တိထည့်ခြင်း
            # ဒေတာထဲတွင် key နာမည်များ ကွဲလွဲနေပါက ရှာမတွေ့တတ်၍ ဖြစ်နိုင်သမျှ နာမည်များအား စုံအောင်ထည့်ထားပါသည်
            row_value = [
                data_dict.get("id") or data_dict.get("blacklist_id") or "",
                data_dict.get("full_name", ""),
                data_dict.get("nrc_number", ""),
                data_dict.get("Remark1") or data_dict.get("company_name") or data_dict.get("remark1") or "",
                data_dict.get("Remark2") or data_dict.get("address") or data_dict.get("remark2") or "",
...
# 🌟 ၂။ Data Update (ပြင်ဆင်) လိုက်လျှင် Google Sheet ထဲရှိ သက်ဆိုင်ရာ Row ကို လိုက်ပြင်ပေးမည့် စနစ်
def auto_sync_update_record(record_id, updated_data_dict):
    ...
    if spreadsheet:
        try:
            ...
            # Google Sheet ထဲရှိ A Column (blacklist_id) များအားလုံးကို ဆွဲထုတ်ခြင်း
            id_list = worksheet.col_values(1) # Column A
            
            # ပြင်ဆင်မည့် ID ၏ Row နံပါတ်ကို ရှာဖွေခြင်း
            str_id = str(record_id)
            if str_id in id_list:
                row_index = id_list.index(str_id) + 1 # Google Sheet Row Index က 1 ကစသဖြင့်
                
                # ပြင်ဆင်လိုက်သော တန်ဖိုးအသစ်များကို Column အလိုက် နေရာချခြင်း
                row_value = [
                    str_id,
                    updated_data_dict.get("full_name", ""),
                    updated_data_dict.get("nrc_number", ""),
                    updated_data_dict.get("Remark1") or updated_data_dict.get("company_name") or updated_data_dict.get("remark1") or "",
                    updated_data_dict.get("Remark2") or updated_data_dict.get("address") or updated_data_dict.get("remark2") or "",
                    updated_data_dict.get("reason", "")
                ]
                
                # 🎯 သက်ဆိုင်ရာ Row တစ်ကြောင်းလုံးကို Google Sheet ပေါ်တွင် ချက်ချင်း ကွက်တိ သွားပြင်လိုက်ခြင်း
                worksheet.update(range_name=f"A{row_index}:F{row_index}", values=[row_value])
                print(f"✨ Real-time UPDATE Sync to Google Sheet Row {row_index} Success!")
            else:
                # အကယ်၍ Sheet ထဲတွင် ရှာမတွေ့ခဲ့ပါက အသစ်အနေဖြင့်ပဲ ထည့်ပေးလိုက်ခြင်း
        ...
        with tab1:
            ...
            if submitted:
                if name and reason:
                    ...
                    # ========================================================
                    # 🌟 [ Add New Record နေရာတွင် ဖြည့်စွက်ရန် ]
                    # ========================================================
                    try:
                        if response.data:
                            # Supabase မှ ကျလာသော id သို့မဟုတ် blacklist_id ကို ကွက်တိ Payload ထဲ ပြန်ထည့်ခြင်း
                            db_id = response.data[0].get("id") or response.data[0].get("blacklist_id")
                            data["id"] = db_id
                        
                        auto_sync_append_record(data)
                    except Exception as sheet_err:
                        print(f"⚠️ Google Sheet Sync Warning: {str(sheet_err)}")
                    ...
                    log_user_activity(
                        ...
                        action=f"Add New Record ({data.get('full_name', 'Unknown')}) & Sync Sheet", 
                        status="Success"
                    )
                    ...
                    msg_container.success(f"🎉 {data.get('full_name', 'Record')} data saved successfully and synced to Google Sheets!")
                ...
                with st.expander(f"{i} 👤 {record['full_name']} ({prefix}: {raw_nrc})"):
                    ...
                    # 👉 [ခ] Edit Mode ဖြစ်နေလျှင် (ဝန်ထမ်း အချက်အလက်ပြင်ဆင်ရန် Form ပြသခြင်း)
                        ...
                        if update_submitted:
                            ...
                            # ၁။ Update လုပ်မည့် ဒေတာ Payload ပြင်ဆင်ခြင်း
                            update_data = {
                            ...
                            # 🎯 လက်ရှိစနစ်သုံးထားသည့် ID အစစ်အမှန်ကို ကွက်တိသတ်မှတ်ခြင်း
                            # (edit_target_id ရှိလျှင် သုံးမည်၊ မရှိပါက record["id"] ကို သုံးမည်)
                            current_id = locals().get('edit_target_id') or record.get('id')

                            # 🟢 Database Table ထဲသို့ Update (ပြင်ဆင်) လုပ်ခြင်း (တစ်ကြောင်းတည်းသာ သုံးပါသည်)
                            response = supabase.table("blacklist_records").update(update_data).eq("id", current_id).execute()
                            
                            # ========================================================
                            # 🌟 [ Data Update လုပ်သည့် နေရာတွင် ဖြည့်စွက်ချက်အမှန် ]
                            # ========================================================
                            try:
                                # ပြင်ဆင်လိုက်သော ID နှင့် ဒေတာအသစ်များကို Sheet ဆီသို့ လှမ်းပို့ခြင်း ✨
                                auto_sync_update_record(current_id, update_data)
                            except Exception as sheet_err:
                                print(f"⚠️ Google Sheet Update Sync Warning: {str(sheet_err)}")
